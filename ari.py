...
if __name__ == '__main__':
    val = torch.randn(2,10,5)
    val_max = torch.argmax(val,-1)
    val_one_hot = F.one_hot(val_max,5)
    val_pred = torch.randn(2,10,5)
    val_pred_max = torch.argmax(val_pred,-1)
    val_pred_one_hot = F.one_hot(val_pred_max,10)    
    # st()
    val_one_hot_new = torch.cat([val_one_hot,val_one_hot],axis=-1)
    st()

    value = adjusted_rand_index(val_one_hot_new,val_pred_one_hot)
    print(value)
    val_one_hot = torch.cat([val_one_hot,torch.zeros_like(val_one_hot)],dim=1)
    val_pred_one_hot = torch.cat([val_pred_one_hot,torch.zeros_like(val_pred_one_hot)],dim=1)
    value = adjusted_rand_index(val_one_hot,val_pred_one_hot)
    print(value)
    st()
    print("hello")
