...
if __name__ == "__main__":
    ...
    randomize_index = np.arange(4)
    random.shuffle(randomize_index)
    pred = torch.randn(3,4,5)
    gt = pred[:,randomize_index]
    # target = torch.randn(3,4,5)
    new_indices = hungarianMatcher(pred,gt)
    gt_updated = []
    pred_updated = []
    # st()
    for ind_ex, indices_ex in enumerate(new_indices):
        p_indices,gt_indices = indices_ex
        gt_updated.append(gt[ind_ex,gt_indices])
        pred_updated.append(pred[ind_ex,p_indices])
        # st()
    # st()

    # st()
    gt_updated = torch.stack(gt_updated)
    pred_updated = torch.stack(pred_updated)
